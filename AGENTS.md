...
* Use CSS grid + flexbox combinations to keep components responsive without breaking the fixed-frame illusion. Test at 1280x720 and tablet breakpoints.
...
## Current Core Loop
* The v1 play loop is chart-first: select an asset, read the live tape, enter with size, then manage the position through saved stop-loss and take-profit rules.
* Opportunity cost is part of the design language. The right-side briefing should keep comparing the current focus against the session leader so "holding dead money" is visible.
* Risk plans are persistent per asset for the current run. If no position is open, saved exits should remain visible as the default for the next entry.
* Insider knowledge is a future-facing edge system. Surface when it is active, but keep the base loop understandable without it.
