...
(function testMarginBuy(){
  ...
  const fee = Math.max(ctx.state.minFee, 10 * price * ctx.state.feeRate);
  const expectedCash = 10000 - (10 * price / 10) - fee;
  assert(Math.abs(ctx.state.cash - expectedCash) < 1e-6, 'cash should reduce by initial margin and fee');
})();
