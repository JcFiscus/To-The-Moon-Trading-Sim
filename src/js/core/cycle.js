...
export function endDay(ctx, cfg=CFG, hooks){
  ...
  const endNet  = ctx.state.cash + endPort - ctx.state.debt;
  const dNet = endNet - ctx.day.startNet;
  const dNetPct = (ctx.day.startNet > 0) ? (endNet/ctx.day.startNet - 1) : 0;

  const meta = {
    day: ctx.day.idx,
    endNet, startNet: ctx.day.startNet,
    dNet, dNetPct,
